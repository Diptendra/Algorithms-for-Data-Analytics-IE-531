...
def assign_probabilities_to_unfair_three_sided_dice():
    # FILL CODE HERE
    # Geneating a random probability for each dice roll with contraint p1 + p2 + p3 = 1
    prob_of_one = np.random.uniform(low=0, high=1)
    prob_of_two = np.random.uniform(low=0, high=(1 - prob_of_one))
...
def simulate_fair_coin_from_unfair_three_sided_dice(p1, p2, p3):
    ...
    roll1 = toss_of_three_sided_unfair_dice(p1, p2, p3)
    roll2 = toss_of_three_sided_unfair_dice(p1, p2, p3)
    roll3 = toss_of_three_sided_unfair_dice(p1, p2, p3)

    # Repeat the process if any two roll of the dice are the same
    while ((roll1 == roll2) or (roll1 == roll3) or (roll2 == roll3)):
        roll1 = toss_of_three_sided_unfair_dice(p1, p2, p3)
        roll2 = toss_of_three_sided_unfair_dice(p1, p2, p3)
        roll3 = toss_of_three_sided_unfair_dice(p1, p2, p3)

    if (roll1 == 1):
        if ((roll2 == 2) and (roll3 == 3)):
            return HEADS
    if (roll1 == 2):
        if ((roll2 == 1) and (roll3 == 3)):
            return HEADS
    if (roll1 == 3):
        if ((roll2 == 2) and (roll3 == 1)):
            return HEADS
...
plt.savefig("UIID_Histogram_dbagchi2.pdf", bbox_inches='tight')
...
plt.savefig("Unit_Normal_Histogram_dbagchi2.pdf", bbox_inches='tight')
